...
먼저, 트랜잭션 전파 방식은 현재 트랜잭션 안에서 새로 호출된 메서드가 같은 트랜잭션을 쓸지, 새 트랜잭션을 만들지를 결정할 때 사용한다
...
오늘은 책 공부 내용은 아니고 회사에서의 트러블 슈팅을 메모해보려고 한다

기존 문제 (As-is)  
비즈니스 로직에서 예외(Exception)를 던지지 않고, Map에 상태값과 메시지를 담아 반환하는 방식  
이 방식은 결제 실패 시 트랜잭션 롤백이 제대로 적용되지 않고, 보상 처리를 일괄적으로 처리하기 어려웠음

개선 방향 (To-be)  
비즈니스 로직에서 런타임 예외를 던지도록 수정  
예외 발생 시 트랜잭션이 자동으로 롤백되도록 설계했고, 컨트롤러 단에서 예외를 일괄 처리하기 수월해짐

개선 중 문제  
결제는 성공하든 실패하든 결제 로그/정보를 DB에 남겨야 해서  
기존 트랜잭션 안에서 예외가 발생하면 insert도 롤백되기 때문에, 결제 로그 저장 메서드에 @Transactional(propagation = REQUIRES_NEW) 적용  
-> 메인 트랜잭션과 분리된 새로운 트랜잭션에서 insert가 수행되어 롤백되지 않도록 처리

위 작업으로 인한 문제 (캐싱으로 인한 조회 결과 null 반환)  
...
#### @ControllerAdvice  
여러 컨트롤러에서 공통 로직 주입을 위한 전역 조언(Advice)을 제공하는 어노테이션  
Spring 내부적으로 AOP 개념을 빌려 Controller 레벨에서 특정 로직을 가로채 공통 동작을 하도록 해주는 구조

가장 일반적인 용도 - 전역 예외 처리 (feat: @ExceptionHandler)  
예외가 발생했을 때 공통 로직으로 처리할 수 있도록 하며, 응답 메시지를 일관적으로 반환하기 용이하다 



#### @ExceptionHandler  
특정 컨트롤러나 전역에서 발생한 예외를 잡아서 처리하는 메서드에 붙이는 어노테이션  
적용 범위는 해당 컨트롤러 클래스 내에서만 유효하며, @ControllerAdvice 전역 컨트롤러에서 사용해 전역 예외 처리를 할 수 있다  

Controller 메서드 수행 중 예외 발생 시 발생한 예외 타입을 보고, 해당 컨트롤러 (없으면 @ControllerAdvice 전역을 스캔) 에 등록된 @ExceptionHandler 중 일치하는 타입 찾고 호출

이 메서드의 반환타입은 ResponseEntity, String, ModelAndView 등으로 자유로우며  
...
#### 추상화를 사용하지 않을 때 생기는 불편함 정리

소프트웨어 설계에서 추상화는 복잡한 구현을 숨기고 핵심 개념만 드러내는 기술이다  
추상화를 사용하지 않으면 어떤 문제가 생기는지 정리해보자

#### 추상화란?

추상화란  
구현 세부사항을 감추고, 공통된 동작이나 개념만을 상위 레벨에서 정의해두는 것  
-> 객체지향, 계층 구조 설계, 인터페이스 설계의 핵심 개념이다
...
추상화가 없는 경우, 개발자는 모든 세부사항을 직접 다뤄야 하므로 다음과 같은 문제가 발생한다

1) **구현 의존 증가**  
   구현 클래스에 직접 의존하게 되어 내부 변경 시 호출부 전체를 수정해야 한다 
   -> 결합도가 상승하며 유지보수가 어려워진다

2) **코드 중복 증가**  
   공통 기능을 묶을 추상 계층이 없으므로 비슷한 로직을 여러 곳에서 중복 작성하게 된다

3) **확장성 저하**  
   기능 추가가 필요한 경우 기존 코드를 계속 수정해야 한다  
   -> 확장보다는 변경 중심 구조가 되어 OCP(Open-Closed Principle)를 위반한다

4) **테스트의 어려움**  
   인터페이스나 추상 타입이 없으면 mock/stub으로 교체하여 테스트하기 힘들다  
   -> 단위 테스트 작성 난이도가 상승한다

5) **계층 구조 붕괴 위험**  
   레이어 간 구분이 흐려지고  
   서비스 -> 리포지토리 -> DB 구조가 무너져 각 레이어가 서로의 구체 구현을 직접 호출하기 쉬워진다


#### 실제로 어떤 문제가 생길까?

- 리포지토리 변경(MySQL -> PostgreSQL 등) 시 서비스 로직까지 수정이 필요해짐
- 외부 API(결제, 인증 등) 변경이 발생하면 비즈니스 코드 전체가 수정됨
- 새로운 기능 추가 요청 시 기존 코드를 직접 뜯어고쳐야 해서 버그 발생률이 증가함
- 작은 수정이 전체 시스템에 영향을 주는 연쇄 폭발 현상이 발생함
...
회사 SI 프로젝트 중 정말 거의 모든 비즈니스 로직이 쿼리에 들어있는 레거시 프로젝트가 있는데,  
정말 유지보수가 너무 힘들다  

요즘은 MSA 컴포넌트 책 진도를 잘 못 나가고, 하루하루 일하면서 부족했던 점이나 트러블슈팅 내용을 여기 정리해두는 정도만 하고 있었다  
이런 저런 이유로 MSA 책 공부를 잠시 중단하고 12월에는 미루었던 JPA 공부를 다시 시작하려고 한다  
약 700페이지 분량의 아주 유명한 영한이형의 `자바 ORM 표준 JPA 프로그래밍` 책이다  
목표는 한 달에서 한 달 반 정도 안에 완독하는 것  
JPA 딥다이브(기본기 탄탄하게), 그리고 객체지향 설계/프로그래밍 실력 향상에 집중하려고 한다  
